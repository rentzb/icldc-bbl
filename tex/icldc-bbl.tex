...
\item Domains of use fall along a scale (see e.g., \citealt{blommaertsociolinguistic2007}) where Tsova-Tush is used the least at translocal levels (doctor and work) and increasingly more often in more local levels. 
\item Even in the most local levels, with spouse or parents, Tsova-Tush is either \textit{always} or \textit{never} used.
...
{\begin{singlespace}\scriptsize We thank the Bilinski Education Foundation for grant support and Dr.~Andrea Berez-Kroeker for comments. Thanks also to Revaz Orbertišvili and to Tsova-Tush respondents: {\georgianfont მადელ შუნ}. Errors are our own.}
\end{singlespace}
}
